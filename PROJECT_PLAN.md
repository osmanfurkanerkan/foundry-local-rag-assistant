...
**Sonradan güncelleme 1 (görsel yeniden tasarım):** ilk sürüm işlevsel ama "çok sade/sabit bir proje gibi" bulundu. Sol panelli (marka + "Yeni Sohbet" + hakkında + bilgi tabanı etiketleri + "100% yerel" rozeti), gradient renk paleti, kullanıcı/asistan avatarları, animasyonlu "yazıyor" noktaları, kaynakları pill/etiket olarak gösterme ve tıklanabilir örnek soru çipleri içeren bir "boş durum" ekranı olan bir ürün-tarzı arayüze yeniden tasarlandı. Gerçek tarayıcıda tekrar test edildi (yeni sohbet sıfırlama dahil).

**Sonradan güncelleme 2 (gerçek bir donanım kısıtı bulundu):** kullanıcı "neden sadece 1-2 soru sorabiliyoruz" diye sordu; canlı teşhiste kök neden bulundu: **CUDA out of memory**. Web arayüzü, CLI'nin aksine sohbet geçmişini sınırsız büyütüp her istekte tamamını API'ye gönderiyordu; büyüyen prompt (ve GPU'daki KV-cache) küçük/sınırlı VRAM'de birkaç turdan sonra belleği taşırdı -- bir test senaryosunda sadece 2 önceki turda bile oluştu. Düzeltme: `api.py`'a `main.py` ile aynı `HISTORY_LIMIT` sunucu tarafında da uygulandı (istemci ne gönderirse göndersin kırpılıyor), `HISTORY_LIMIT` 5'ten 3'e düşürüldü, ve CUDA/OOM hatası yakalanıp kullanıcıya ham stack trace yerine "Foundry Local'i yeniden başlat" talimatı içeren anlaşılır bir mesaj gösteriliyor. **Dürüst not:** bu tam bir çözüm değil, sadece riski azaltıyor -- düşük VRAM'li GPU'larda uzun sohbetlerde OOM hâlâ oluşabilir; bu, küçük/yerel donanımın gerçek bir sınırı.
