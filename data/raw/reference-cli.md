...
Kaynak: https://learn.microsoft.com/en-us/azure/foundry-local/reference/reference-cli
...
[![Screenshot of output from Foundry Local run command.](../media/get-started-output.png)](../media/get-started-output.png#lightbox)
...
| `OpenVINOExecutionProvider` (Intel) | CPU: Intel TigerLake (11th Gen) and later versions with min recommended driver 32.0.100.9565 GPU: Intel AlderLake (12th Gen) and later versions with min recommended driver 32.0.101.1029 NPU: Intel ArrowLake (15th Gen) and later versions with min recommended driver 32.0.100.4239 | [Intel OBL Distribution Commercial Use License Agreement v2025.02.12](https://cdrdv2.intel.com/v1/dl/getContent/849090?explicitVersion=true) |
| `QNNExecutionProvider` (Qualcomm) | Snapdragon(R) X Elite - X1Exxxxx - Qualcomm(R) Hexagon(TM) NPU with minimum driver version 30.0.140.0 and later versions Snapdragon(R) X Plus - X1Pxxxxx - Qualcomm(R) Hexagon(TM) NPU with minimum driver version 30.0.140.0 and later versions | To view the QNN License, download the Qualcomm® Neural Processing SDK, extract the ZIP, and open the LICENSE.pdf file. |
| `VitisAIExecutionProvider` (AMD) | Min: Adrenalin Edition 25.6.3 with NPU driver 32.00.0203.280 Max: Adrenalin Edition 25.9.1 with NPU driver 32.00.0203.297 | No additional license required |
...
3. Install and launch [Open WebUI](https://github.com/open-webui/open-webui), then open `http://localhost:8080` in your browser.
4. Connect Open WebUI to Foundry Local:

   1. Go to **Settings** > **Admin Settings** > **Connections** and enable **Direct Connections**.
   2. Go to **Settings** > **Connections** > **Manage Direct Connections** and select **+**.
   3. Set **URL** to `http://localhost:PORT/v1` (replace `PORT` with the port from step 2) and **Auth** to **None**.
   4. Select **Save**.
...
For more troubleshooting guidance, see [Best practices and troubleshooting](reference-best-practice).
